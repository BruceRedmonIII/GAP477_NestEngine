...
#include <backends/imgui_impl_vulkan.h>
	...
	bool AppStartup(SDL_Window* pWindow, const char* exePath)
	{
		...
		cubeIndexCount = _countof(cubeIndices);
		cubeIndexBuffer = vulkan.CreateIndexBuffer(sizeof(cubeIndices), cubeIndices);
		cubeVertexBuffer = vulkan.CreateBuffer(vk::BufferUsageFlagBits::eVertexBuffer, sizeof(cubeVertices), cubeVertices);
	...
	void AppOnTick(float tickDelta)
	{
		...
		commands.pushConstants(gridPipeline->GetLayout(), vk::ShaderStageFlagBits::eAllGraphics, 0, sizeof(pushConstants), &pushConstants);
