...
std::tuple<vk::Buffer, vk::Buffer, uint32_t> LoadCubeAsset()
{
	...
	const aiScene* scene = importer.ReadFile("demos/06-basic-lighting/icosphere.glb", aiProcess_MakeLeftHanded);
	if (!scene)
	{
	...
	for (uint32_t v = 0; v < mesh->mNumVertices; ++v)
	{
		...
		if (mesh->HasNormals())
		{
			...
			vertices.push_back({ { vert.x, vert.y, vert.z }, { normal.x, normal.y, normal.z } });
		}
		else
		{
			vertices.push_back({ { vert.x, vert.y, vert.z }, glm::normalize(glm::vec3{ vert.x, vert.y, vert.z }) });
		}
	}
...
bool AppStartup(SDL_Window* pWindow, const char* exePath)
{
	...
	{
		{ {  0.0f,  0.5f,  0.0f }, {  0.000f,  1.000f,  0.000f } },
